...
const resolvers = {
  Query: {
    ...
    announcements: async (_, { role }, { user }) => {
      // Public announcements + role‑specific
      let query = `SELECT * FROM announcements WHERE targetRole IN ('all', ?) OR targetRole IS NULL`;
      const [rows] = await db.query(query, [role || 'all']);
      return rows;
    },
  ...
  Mutation: {
    ...
    replyToQuery: async (_, { queryId, reply }, { user }) => {
      requireRole(user, ['lecturer']);
      // Verify the query belongs to a course taught by this lecturer
    ...
    assignGrade: async (_, { studentId, courseId, grade }, { user }) => {
      requireRole(user, ['lecturer']);
      // Verify lecturer teaches the course
      const [course] = await db.query(`SELECT lecturer_id FROM courses WHERE id = ?`, [courseId]);
      if (course.length === 0 || course[0].lecturer_id !== user.id)
        throw new Error('Not authorized for this course');
      await db.query(
    ...
    // Admin mutations (simplified – implement fully as needed)
    ...
    updateUserRole: async (_, { userId, roleNames }, { user }) => {
      ...
      return { ...updatedUser[0], roles: roleNames.map(r => ({ name: r })) };
    },
